...
const compress_tile = tile => {
  ...
  export const decompress_tile = tile => {
...
//   console.log('gre', get_random_element(test))
  ...
  const island_tiles = island => {
    for(let row=0; row<island.length; row++) {
        for(let col=0; col<island[0].length; col++) {
            ...
            if(tile_type === 2) {
                island[row][col] = get_shore_tile(island, col, row)
            }
        }
    }
  }
  ...
  export const universe_variables = {
...
export const generate_map = (tileset, config) => {
...
export const draw_tile = (ctx, coordinates, pixel_size, colors, data) => {
...
export const draw_grid = (ctx, universe_variables) => {
    for(let line=0;line<universe_variables.map_width; line+=8) {
        ctx.fillStyle = '#000'
        ctx.fillRect(line, 0, 1, universe_variables.map_width) //verticle
        ctx.fillRect(0, line, universe_variables.map_width, 1) //horizontal
    }
}
